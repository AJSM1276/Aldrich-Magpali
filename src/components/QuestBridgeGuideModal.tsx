    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-[#E7DFD3] dark:border-stone-800 flex items-center justify-between bg-[#FAF7F2] dark:bg-[#1A1224] transition-colors">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-white rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors text-xs flex items-center gap-1 cursor-pointer"
              title="Share Guide Link"
            >
              {copiedLink ? <Check className="w-4 h-4 text-emerald-700 dark:text-emerald-400" /> : <Share2 className="w-4 h-4" />}
            </button>
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-12 overflow-hidden">
          ...
          <div className="md:col-span-4 border-r border-[#E7DFD3] dark:border-stone-800 overflow-y-auto p-4 space-y-2 bg-[#FAF7F2] dark:bg-[#1A1224] transition-colors">
            <div className="text-[10px] font-medium uppercase tracking-wider text-stone-500 dark:text-stone-400 px-2 mb-2">
              Table of Contents
            </div>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl transition-all border flex items-center justify-between cursor-pointer ${
                    isSelected
                      ? 'bg-white dark:bg-[#281C37] border-stone-800 dark:border-stone-600 text-stone-900 dark:text-white shadow-2xs font-medium'
                      : 'bg-white/80 dark:bg-[#1F162B] border-stone-200/70 dark:border-stone-800/80 text-stone-700 dark:text-stone-300 hover:border-stone-300 dark:hover:border-stone-700 hover:bg-white dark:hover:bg-[#251B33]'
                  }`}
                >
                  <div>
                    <span className="text-[10px] font-medium uppercase text-stone-500 dark:text-stone-400 block">
                      Chapter {idx + 1}
                    </span>
                  ...
                  <ChevronRight className={`w-4 h-4 shrink-0 transition-transform ${isSelected ? 'text-stone-900 dark:text-white translate-x-0.5' : 'text-stone-300 dark:text-stone-600'}`} />
                </button>
              );
            })}
          </div>
          ...
          <div className="md:col-span-8 overflow-y-auto p-6 sm:p-10 bg-white dark:bg-[#120B1C] space-y-6 transition-colors">
              ...
              >
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2.5 py-0.5 text-[10px] font-medium uppercase rounded-full bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 border border-stone-200 dark:border-stone-700">
                      Chapter {activeChapterIdx + 1} of {QUESTBRIDGE_PLAYBOOK.length}
                    </span>
                  </div>

                  <h2 className="font-serif text-2xl sm:text-3xl font-normal text-stone-900 dark:text-white leading-tight">
                ...
                <div className="p-4 sm:p-5 rounded-xl bg-[#FAF7F2] dark:bg-[#1A1224] border border-[#E7DFD3] dark:border-stone-800 transition-colors">
                  <h4 className="text-[10px] font-medium uppercase tracking-wider text-stone-600 dark:text-stone-400 mb-2.5 flex items-center gap-1.5">
                    <Lightbulb className="w-3.5 h-3.5 text-[#876834] dark:text-[#E2C799]" />
                    <span>Tactical Strategy Rules</span>
                  </h4>
                  <ul className="space-y-2">
                    {currentChapter.keyTips.map((tip, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs sm:text-sm text-stone-800 dark:text-stone-200">
            ...
            <div className="pt-6 border-t border-stone-100 dark:border-stone-800 flex items-center justify-between">
              <button
                ...
                className="px-4 py-2 text-xs font-medium border border-stone-200 dark:border-stone-700 rounded-lg disabled:opacity-30 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors cursor-pointer text-stone-700 dark:text-stone-300"
              >
                Previous Chapter
              </button>
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-medium bg-[#241D2B] dark:bg-[#3E2B4E] hover:bg-[#382B42] dark:hover:bg-[#4E3862] text-white rounded-lg disabled:opacity-30 transition-colors shadow-2xs cursor-pointer"
              >
                Next Chapter
              </button>
            </div>
